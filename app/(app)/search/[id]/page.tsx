...
function DetailField({ label, value }: { label: string; value: string | null | undefined }) {
  return (
    <Box sx={{ p: 1.5, border: '1px solid', borderColor: 'divider', borderRadius: '18px' }}>
...
function normalizeOutreachPhone(value: string | null | undefined) {
...
export default function VoterProfilePage() {
  ...
  const [templates, setTemplates] = useState<FieldContentTemplate[]>(() => getDefaultFieldTemplates());
    ...
    const run = async () => {
      ...
      if (localVoter) {
        ...
        if (localVoter.house_no) {
          const list = await db.voters.where('house_lc').equals((localVoter.house_no ?? '').toLowerCase()).limit(50).toArray();
      ...
      try {
        ...
        const json = (await res.json().catch(() => null)) as
          | { error?: string; voter?: OfflineVoter; relatives?: OfflineVoter[] }
          | null;
        ...
        const resolvedRelatives = ((json.relatives ?? []) as OfflineVoter[]).map(enrich);
        ...
        setLoadError(error instanceof Error ? error.message : 'Failed to load voter');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  useEffect(() => {
    const loadTemplates = async () => {
      try {
        ...
        const json = (await res.json().catch(() => null)) as { templates?: FieldContentTemplate[] } | null;
  ...
  const whatsappTemplate = useMemo(() => templateByType(templates, 'WHATSAPP'), [templates]);
  ...
  const saveEdits = async (patch: Partial<EditableVoterFields>) => {
    if (!voter) return;
    const nextVoter = enrich({ ...voter, ...patch, updated_at: new Date().toISOString() });
  ...
  return (
    <Stack spacing={2}>
      <Card>
        <CardContent sx={{ p: 2 }}>
          ...
          <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>
            House: {voter.house_no ?? '-'} · Phone: {voter.mobile_no ?? '-'} · Age: {voter.age ?? '-'}
          </Typography>
          ...
          >
            <Button
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  console.error(error);
                  setStatus('Bluetooth printing failed. Ensure printer is on and paired.');
                }
              }}
            >
              Bluetooth Print
            ...
            <Button
              ...
              onClick={async () => {
                try {
                  ...
                  const file = new File([blob], `voter-${voter.epic_id || voter.id}.png`, {
                    type: 'image/png'
                  });
                  ...
                  setStatus('Share not supported. Text copied and image downloaded.');
                } catch (error) {
            ...
            <Button
              ...
              onClick={async () => {
                ...
                const url = `sms:${phone}?body=${encodeURIComponent(outreachText)}`;
                ...
                try {
                  ...
                  if (contentType && contentType.includes("application/json")) {
                    ...
                    if (!res.ok || !json.success) {
                      throw new Error(JSON.stringify(json.error) || 'Failed to send WhatsApp message');
                    }
                  } else {
        ...
        <CardContent sx={{ p: 2 }}>
          <Stack direction="row" justifyContent="space-between" alignItems="center" spacing={1.5}>
            ...
            <Stack direction="row" spacing={1}>
              <Button variant="outlined" startIcon={<EditRoundedIcon />} onClick={() => openEditor('contact')}>
                Edit Contact
              </Button>
              <Button variant="outlined" startIcon={<EditRoundedIcon />} onClick={() => openEditor('notes')}>
                Edit Notes
              </Button>
            </Stack>
          </Stack>
      ...
      <Card>
        <CardContent sx={{ p: 2 }}>
          <Stack direction="row" justifyContent="space-between" alignItems="center" spacing={1.5}>
            ...
            <Button variant="outlined" startIcon={<EditRoundedIcon />} onClick={() => openEditor('enrichment')}>
              Edit Details
            </Button>
          </Stack>
          ...
          <Grid container spacing={1.5} sx={{ mt: 1 }}>
            ...
            <Grid item xs={12} sm={6}>
              <DetailField label="Aadhar Card No" value={voter.aadhar_card_no} />
            </Grid>
            <Grid item xs={12} sm={6}>
            ...
            <Grid item xs={12}>
              <DetailField label="Interested Party" value={voter.interested_party} />
            </Grid>
          </Grid>
        </CardContent>
      </Card>
      ...
      <Dialog open={editorMode !== null} onClose={() => setEditorMode(null)} fullWidth maxWidth="sm">
        <DialogTitle>
          {editorMode === 'contact'
            ? 'Edit Contact'
            : editorMode === 'notes'
          ...
          <Stack spacing={1.5}>
            {editorMode === 'contact' && (
              <TextField
                ...
                onChange={(e) => setDraft((prev) => ({ ...prev, mobile_no: e.target.value }))}
                fullWidth
              />
            )}
            ...
            {editorMode === 'enrichment' && (
              <>
                <TextField
                  label="Caste"
                  value={draft.caste ?? ''}
                  onChange={(e) => setDraft((prev) => ({ ...prev, caste: e.target.value }))}
                  fullWidth
                />
                <TextField
                  label="Religion"
                  value={draft.religion ?? ''}
                  onChange={(e) => setDraft((prev) => ({ ...prev, religion: e.target.value }))}
                  fullWidth
                />
                <TextField
                  label="Aadhar Card No"
                  value={draft.aadhar_card_no ?? ''}
                  onChange={(e) => setDraft((prev) => ({ ...prev, aadhar_card_no: e.target.value }))}
                  fullWidth
                />
                <TextField
                  label="Education"
                ...
                <TextField
                  ...
                  onChange={(e) => setDraft((prev) => ({ ...prev, local_issue: e.target.value }))}
                  multiline
                  minRows={3}
                  fullWidth
                />
                <TextField
                  ...
                  onChange={(e) => setDraft((prev) => ({ ...prev, interested_party: e.target.value }))}
                  fullWidth
                />
              </>
            )}
        ...
        <DialogActions sx={{ px: 2, py: 1.5 }}>
          <Button variant="outlined" onClick={() => setEditorMode(null)} disabled={isSaving}>
            Cancel
          </Button>
          <Button variant="contained" onClick={() => void submitEditor()} disabled={isSaving}>
      ...
      >
        ...
        >
          <Box sx={{ px: 3, py: 2, bgcolor: '#1976d2', color: '#fff' }}>
            <Typography sx={{ fontSize: 28, fontWeight: 800 }}>Voter Details</Typography>
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={6}>
                <Box sx={{ p: 2, border: '1px solid #e5e7eb', borderRadius: 2 }}>
                  <Typography sx={{ fontSize: 12, color: '#6b7280' }}>EPIC ID</Typography>
              ...
              <Grid item xs={6}>
                <Box sx={{ p: 2, border: '1px solid #e5e7eb', borderRadius: 2 }}>
                  <Typography sx={{ fontSize: 12, color: '#6b7280' }}>House No</Typography>
              ...
              <Grid item xs={6}>
                <Box sx={{ p: 2, border: '1px solid #e5e7eb', borderRadius: 2 }}>
                  <Typography sx={{ fontSize: 12, color: '#6b7280' }}>Phone</Typography>
                  <Typography sx={{ mt: 0.5, fontSize: 20, fontWeight: 700 }}>
                    {voter.mobile_no ?? '-'}
                  </Typography>
                </Box>
              ...
              <Grid item xs={6}>
                <Box sx={{ p: 2, border: '1px solid #e5e7eb', borderRadius: 2 }}>
                  <Typography sx={{ fontSize: 12, color: '#6b7280' }}>Serial No</Typography>
                  <Typography sx={{ mt: 0.5, fontSize: 20, fontWeight: 700 }}>
                    {voter.serial_no ?? '-'}
                  </Typography>
                </Box>
              ...
              <Grid item xs={6}>
                <Box sx={{ p: 2, border: '1px solid #e5e7eb', borderRadius: 2 }}>
                  <Typography sx={{ fontSize: 12, color: '#6b7280' }}>Age</Typography>
                  <Typography sx={{ mt: 0.5, fontSize: 20, fontWeight: 700 }}>
                    {voter.age ?? '-'}
                  </Typography>
                </Box>
              ...
              <Grid item xs={12}>
                <Box sx={{ p: 2, border: '1px solid #e5e7eb', borderRadius: 2 }}>
                  <Typography sx={{ fontSize: 12, color: '#6b7280' }}>Booth Name</Typography>
